...
rule all:
	input:
                # check
                f'check/prepare_upload_{METADATA_VERSION}',
...
rule prepare_data:
	output:
		'check/prepare_upload_{METADATA_VERSION}'
	container:
	...
	shell:
		'src/upload_prepare_data_AHMeSHDbs.sh {METADATA_VERSION} {output} >& {log}'
...
rule upload:
	...
	shell:
		'src/upload_AHMeSHDbs.sh "{URL}" {output} >& {log}'
